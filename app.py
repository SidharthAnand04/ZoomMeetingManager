...
def working():
    return 'Working'
# define a route for the application that supports POST request
@app.route('/meeting', methods=['POST'])
def meeting():
        ...
        #
            ...
            if api_resp.status_code == 201:
                reply = (
                        ...
                        "\nMeeting Topic:  " + str(meeting_info["topic"]) +
                        "\nMeeting Agenda:  " + str(meeting_info["agenda"]) +
                        "\nMeeting Start Time:  " + str(session["start_time"]) +
                        "\nMeeting Start URL:  " + str(meeting_info["start_url"]))
                msg.body(reply)
                session.pop("topic", None)
                session.pop("start_time", None)
    ...
    if in_msg == "4":
        session.clear()
        msg.body('Meeting has been reset')
        return str(resp)
...
#     #         "\nMeeting Start URL:  " + str(meeting_info["start_url"]))
# else:
#     print("Could not create meeting")
